...
\vspace{0.25cm}
\itshape
These notes provide a structured first complete draft for a graduate lecture note on precision physics at the Future Circular Collider. The emphasis is on the FCC-ee precision programme as a Higgs, electroweak, top, flavour and BSM search machine, and on the way FCC-hh completes the programme at the energy frontier. The text intentionally combines formulas, process definitions, figures, guided checks, examples and exercises. Numerical projections should be checked against the most recent FCC Feasibility Study and ESPPU documents before final public use.
\end{guidelinebox}
...
This lecture note is a first complete, compile-ready draft designed to support a graduate course or advanced seminar on precision physics at the Future Circular Collider. It is based on the FCC Feasibility Study Report, the FCC Conceptual Design Report, the FCC integrated-programme manifesto, recent ECFA Higgs/electroweak/top factory material, top-threshold studies, SMEFT interpretation papers, and curated figures from the local \texttt{Figs/} directory.
...
\begin{equation}
  ...
  \longrightarrow
  \text{overconstrained SM precision tests}
  \longrightarrow
  \text{indirect BSM sensitivity}
  \longrightarrow
  \text{direct exploration at FCC-hh} .
\end{equation}
...
\end{figure}
The figure should be read together with the equations in this section: the experimental projection becomes meaningful only after the observable, the assumptions entering its extraction, and the dominant uncertainties have been identified.
...
\begin{table}[H]
\centering
\caption{Schematic FCC-ee working points and their main precision roles. Numerical yields are rounded and should be checked against the most recent FSR/ESPPU tables before final publication.}
\label{tab:run-plan}
\begin{tabularx}{\textwidth}{@{}p{2.7cm}p{2.5cm}p{3cm}X@{}}
\toprule
Working point & Approx. energy & Main process & Main physics role \\
\midrule
Z pole & 88, 91, 94 GeV & \(e^+e^-\to Z\) & Z lineshape, \(\mZ\), \(\gZ\), asymmetries, \(R_b\), \(R_c\), flavour, tau, rare Z decays, HNLs.\\
WW threshold & 157, 163 GeV & \(e^+e^-\to W^+W^-\) & \(\mW\), \(\gW\), W branching fractions, diboson tests.\\
Higgs factory & 240 GeV & \(e^+e^-\to ZH\) & inclusive \(\sigma_{ZH}\), Higgs recoil, absolute couplings, invisible/exotic decays.\\
Top threshold & 340--350 GeV & \(e^+e^-\to t\bar t\) & threshold scan, \(\mt\), \(\Gt\), \(\als\), top Yukawa sensitivity.\\
High energy & 365 GeV & \(t\bar t\), \(\nu\bar\nu H\) & top electroweak couplings, WW fusion, improved Higgs width/couplings.\\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{guidelinebox}
The electroweak programme is the precision backbone of FCC-ee. The Z pole supplies enormous statistics and exceptionally clean pseudo-observables, while the WW threshold determines the W mass and width. Together with the top and Higgs measurements, these observables form a high-precision closure test of the Standard Model.
\end{guidelinebox}
...
\begin{examplebox}{Sensitivity of an asymmetry to \(\sin^2	heta_{
m eff}\)}{asymmetry-example}
...
\begin{guidelinebox}
The Tera-Z programme turns FCC-ee into a heavy-flavour and tau factory. The clean environment, large boost of heavy hadrons, high statistics and excellent vertexing create opportunities that are complementary to LHCb, Belle II and fixed-target experiments.
\end{guidelinebox}
...
\begin{guidelinebox}
FCC-ee is often described as a precision machine, but precision and direct searches are not separate. The huge Z, W and Higgs samples enable searches for feebly interacting particles, long-lived particles and exotic decays that may be invisible to traditional high-pT searches.
\end{guidelinebox}
...
\begin{guidelinebox}
The detector is part of the precision argument. Vertexing, tracking, calorimetry, particle identification, timing and software all determine whether the theoretical reach of FCC-ee can be realised experimentally.
\end{guidelinebox}
...
\begin{guidelinebox}
FCC-hh extends the FCC programme to the energy frontier. Its role is not only to search for new heavy particles, but also to complete the Higgs programme, test high-energy tails and provide direct access to physics that may first appear indirectly at FCC-ee.
\end{guidelinebox}
...
\begin{tabularx}{\textwidth}{@{}p{2.7cm}p{3.4cm}X@{}}
\toprule
...
\end{enumerate}
For figures taken from FCC reports or source papers, the captions include source attributions. Before public redistribution or publication beyond internal teaching use, the source and licence of each figure should be checked against the original document.
...
\bibitem{ZLineshape2021} J. Alcaraz Maestre, A. Blondel, M. Dam and P. Janot, \emph{The Z lineshape challenge: ppm and keV measurements}, Eur. Phys. J. Plus 136 (2021) 848.
\bibitem{WMassWidth2021} P. Azzurri, \emph{The W mass and width measurement challenge at FCC-ee}, Eur. Phys. J. Plus 136 (2021) 1203.
...
\bibitem{Key4hep2022} G. Ganis et al., \emph{Key4hep, a framework for future HEP experiments and its use in FCC}, Eur. Phys. J. Plus 137 (2022) 149.
