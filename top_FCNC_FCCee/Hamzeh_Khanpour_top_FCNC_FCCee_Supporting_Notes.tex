...
\lhead{\textcolor{fccblue}{\sffamily 
Precision Physics at the FCC-ee}}
...
\author{
	...
	Faculty of Physics and Applied Computer Science \\[0.35cm] AGH University of Krakow
}
...
\centering
\vspace{0.55cm}
{\Huge\bfseries\textcolor{fccblue}{Precision Physics at the FCC-ee}} 
%{\Huge\bfseries\textcolor{fccblue}{}}\\[0.30cm]
...
{\normalsize Faculty of Physics and Applied Computer Science}\\[0.25cm]
{\normalsize {\color{blue}\bfseries AGH University of Krakow}}
\vspace{0.35cm}
\end{minipage}
}
...
\end{equation}
One needs to consider the $\delta g_{1,Z}$, $\delta\kappa_\gamma$ and $\lambda_Z$ as the key deviations.  Here $\delta g_{1,Z}$ modifies the overall $WWZ$ structure, $\delta\kappa_\gamma$ acts like an anomalous magnetic-dipole contribution to $WW\gamma$, and $\lambda_Z$ is a momentum-dependent higher-derivative interaction.  In SMEFT, these parameters are correlated with Higgs-gauge interactions such as $HZZ$ and $HWW$.
...
The SMEFT operators most relevant include bosonic operators and current operators.  Examples are
\begin{align}
...
\end{align}
These can shift $Zee$ couplings, affect $HZZ$ and $HZee$ contact interactions and correlate with diboson and Higgs production.  Bosonic operators such as $\mathcal O_{HW}$, $\mathcal O_{HB}$ and $\mathcal O_W$ affect gauge-boson kinetic/momentum structures and anomalous triple-gauge couplings.  This is the origin of the chords in the circos plot: the plot visualises correlations among fitted effective couplings, not independent measurements (See Fig.~\ref{fig:circos-fccee-global-fit}).



%------------------------------------------------------------
\section{Global Higgs--electroweak interpretation}
\label{sec:global-higgs-ew-before-top}
%------------------------------------------------------------

It is also useful to make explicit why the
...
the chords inside the circle indicate correlations between fitted quantities. A thick chord represents a stronger correlation. The figure should be read in three layers.
...
A full FCC electroweak fit also includes QED radiator functions, beam-energy calibration, luminosity theory and correlations among pseudo-observables.
