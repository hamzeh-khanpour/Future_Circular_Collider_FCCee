...
%----------------------------------------------------------------------------------------
\section{{\color{white}FCC-ee precision physics: EW, Z and Higgs}}

%----------------------------------------------------------------------------------------
\begin{frame}{Why FCC-ee precision physics?}
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.56\textwidth}
\begin{itemize}
\justifying
\small
\item The Higgs boson has been discovered, but the \alert{origin of electroweak symmetry breaking}, the flavour structure, dark matter, neutrino masses and the scale of new dynamics remain open.
\item If new physics is heavy or weakly coupled, its first visible effect may be a \alert{small deviation} in a precision observable or a rare/forbidden decay.
\item FCC-ee provides the luminosity-frontier first stage of the integrated FCC programme: a clean $e^+e^-$ machine for \alert{Higgs, electroweak, flavour, QCD, top and BSM precision}.
\item FCC-hh later extends the same programme to the direct $100~\mathrm{TeV}$ energy frontier.
\end{itemize}
\end{column}
\begin{column}{0.40\textwidth}
\centering
\includegraphics[width=0.84\linewidth]{FCC.jpg}
\vspace{0.15cm}
\begin{block}{Precision-first logic}
\centering\small
absolute EW/Higgs inputs first;\\ direct high-energy exploration later.
\end{block}
\end{column}
\end{columns}
\takehome{FCC-ee should be read as a precision microscope for the full electroweak sector, not only as a Higgs factory.}
\refs{FCC Feasibility Study Report Vol.~1, Eur. Phys. J. C 85 (2025) 1468, Secs.~1--2.}
\end{frame}

%----------------------------------------------------------------------------------------
\begin{frame}{FCC-ee running stages and baseline programme}
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.48\textwidth}
\centering
\includegraphics[width=0.95\linewidth]{FCCeeLumis2024.png}
\vspace{-0.1cm}
\scriptsize Baseline luminosity over four IPs.
\end{column}
\begin{column}{0.50\textwidth}
\small
\begin{tabular}{lcc}
\toprule
Stage & $\sqrt{s}$ & Main sample \\
\midrule
$Z$ pole & 88, 91, 94 GeV & $6\times10^{12}~Z$ \\
$WW$ threshold & 157, 163 GeV & $2.4\times10^{8}~WW$ \\
$ZH$ maximum & 240 GeV & $2.2\times10^{6}~ZH$ \\
Top threshold & 340--350, 365 GeV & $2\times10^{6}~t\bar t$ \\
\bottomrule
\end{tabular}
\vspace{0.25cm}
\begin{itemize}
\justifying
\item Baseline programme: four interaction points and about 15 years of FCC-ee operation.
\item Almost $3\times10^6$ Higgs bosons are produced when the $ZH$ and higher-energy Higgs samples are combined.
\item The stages are mutually reinforcing: EW inputs, Higgs rates, top parameters and BSM searches are fitted together.
\end{itemize}
\end{column}
\end{columns}
\refs{FCC Feasibility Study Report Vol.~1, Fig.~1 and Table~1.}
\end{frame}

%----------------------------------------------------------------------------------------
\begin{frame}{Why FCC-ee can reach unprecedented precision}
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.52\textwidth}
\begin{block}{Machine and event-level advantages}
\small
\begin{itemize}
\justifying
\item clean and colourless $e^+e^-$ initial state;
\item known $\sqrt{s}$ and strong beam-energy control;
\item low pile-up and full-event reconstruction;
\item four IPs: luminosity, redundancy and cross-checks;
\item resonant depolarisation at the $Z$ pole and $WW$ threshold;
\item detector concepts optimised for tracking, flavour tagging, PID and luminosity monitoring.
\end{itemize}
\end{block}
\end{column}
\begin{column}{0.45\textwidth}
\centering
\includegraphics[width=0.86\linewidth]{FCCeeSequenceBaseline.pdf}
\vspace{0.05cm}
\begin{block}{Key point}
\centering\small
The challenge is not only statistics; it is matching systematic and theory precision to huge data samples.
\end{block}
\end{column}
\end{columns}
\refs{FCC Feasibility Study Report Vol.~1, Fig.~2; Secs.~1.1, 3, 4 and 9.}
\end{frame}
...
\end{column}
\end{columns}
\refs{FCC Feasibility Study Report Vol.~1, Table~2.}
\end{frame}
...
\begin{column}{0.54\textwidth}
\centering
\includegraphics[width=0.98\linewidth]{circos_FCCee_nomargin_newFSR.pdf}
\end{column}
\begin{column}{0.43\textwidth}
\begin{itemize}
\justifying
...
%----------------------------------------------------------------------------------------
\begin{frame}{Detector and theory requirements for precision}
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.48\textwidth}
\begin{block}{Detector/control requirements}
\small
\begin{itemize}
\justifying
\item luminosity and angular acceptance;
\item beam-energy scale and spread;
\item tracker momentum scale and alignment;
\item vertexing, $b/c/s$ tagging and PID;
\item lepton ID and calorimeter granularity;
\item jet reconstruction and missing-energy control.
\end{itemize}
\end{block}
\end{column}
\begin{column}{0.49\textwidth}
\begin{block}{Theory/calculation requirements}
\small
\begin{itemize}
\justifying
\item QED, EW and QCD higher-order corrections;
\item event generators at the required precision;
\item pseudo-observable definitions and correlations;
\item quark-mass and strong-coupling inputs;
\item robust SMEFT and global-fit interpretation.
\end{itemize}
\end{block}
\end{column}
\end{columns}
\takehome{The FCC-ee physics case is as much about controlling systematics and theory as about collecting huge event samples.}
\refs{FCC Feasibility Study Report Vol.~1, Secs.~3, 4 and 9.}
\end{frame}

%----------------------------------------------------------------------------------------
\begin{frame}{FCC-ee and FCC-hh synergy}
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.50\textwidth}
\begin{itemize}
\justifying
\small
\item FCC-ee supplies clean EW inputs, absolute $ZH$ normalisation, Higgs width information and precision top/EW constraints.
\item FCC-hh adds enormous Higgs statistics, rare Higgs modes, top Yukawa precision, double-Higgs production and direct high-mass discovery reach.
\item Together they form a consistent programme: precision first, energy frontier next.
\end{itemize}
\begin{alertblock}{Complementarity}
\small
FCC-ee calibrates the Higgs/EW picture; FCC-hh extends it to the multi-TeV frontier.
\end{alertblock}
\end{column}
\begin{column}{0.47\textwidth}
\centering
\includegraphics[width=0.49\linewidth]{ZttHttplot_FCCee.pdf}
\includegraphics[width=0.49\linewidth]{H3Httplot_FCCee_noHL.pdf}
\vspace{0.1cm}
\scriptsize FCC-ee inputs improve FCC-hh top-Yukawa and Higgs self-coupling interpretations.
\end{column}
\end{columns}
\refs{FCC Feasibility Study Report Vol.~1, Sec.~2.2.4.}
\end{frame}

%----------------------------------------------------------------------------------------
\begin{frame}{Bridge to this seminar: rare top interactions at FCC-ee}
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.52\textwidth}
\begin{itemize}
\justifying
\small
\item The precision programme searches for small deviations in well-controlled observables.
\item Top FCNC interactions are a natural rare-process benchmark: absent at tree level in the SM and extremely suppressed by the GIM mechanism.
\item FCC-ee offers the same ingredients that make the Higgs/EW programme powerful: known initial state, clean event topology, precise reconstruction and controllable backgrounds.
\item The relevant production mode is $e^+e^-\to tq$ with anomalous $tq\gamma$ and $tqZ$ interactions.
\end{itemize}
\end{column}
\begin{column}{0.44\textwidth}
\begin{block}{Transition}
\small
With the FCC-ee precision context in mind, we now turn to one concrete rare-top benchmark: single-top FCNC production $e^+e^-\to tq$ at FCC-ee.
\end{block}
\vspace{0.25cm}
\begin{center}
\includegraphics[width=0.72\linewidth]{FCNC.jpg}
\end{center}
\end{column}
\end{columns}
\refs{FCC Feasibility Study Report Vol.~1, Secs.~1--2; Top-FCNC FCC-ee analysis note.}
\end{frame}
